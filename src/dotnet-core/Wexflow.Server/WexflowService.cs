    ...
    public sealed class WexflowService : NancyModule
    {
        ...
        private void IsWorkflowIdValid()
        {
            Get(Root + "isWorkflowIdValid/{id}", args =>
            {
                ...
                var falseStr2 = JsonConvert.SerializeObject(false);
                var falseBytes2 = Encoding.UTF8.GetBytes(falseStr2);

                return new Response()
                {
                    ContentType = "application/json",
                    Contents = s => s.Write(falseBytes2, 0, falseBytes2.Length)
                };
            });
        }
                ...
                try
                {
                    ...
                    if (user.Password.Equals(password))
                    {
                        ...
                        new Core.Workflow(
                              ...
                              , WexflowServer.WexflowEngine.WorkiomAuthUrl
                              , WexflowServer.WexflowEngine.CreateRecordUrl
                              , WexflowServer.WexflowEngine.UpdateRecordUrl
                              , WexflowServer.WexflowEngine.NotifyUserUrl
                            );
